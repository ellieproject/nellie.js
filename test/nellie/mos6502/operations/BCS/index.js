/* test/nellie/mos6502/operations/BCS/index.js
 *
 */
...
test.beforeEach((t) => {
  ...
  t.context.OP = require('@ellieproject/nellie/mos6502/operations/BCS');
});
...
test('execute() should alter PC when C is set (add)', (t) => {
...
test('execute() should alter PC when C is set (add page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('C', 1);
...
test('execute() should alter PC when C is set (sub)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('C', 1);
...
test('execute() should alter PC when C is set (sub page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('C', 1);
...
test('execute() should not alter PC when C is clear', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
