/* test/nellie/mos6502/operations/BVS/index.test.js
 *
 */
...
test.beforeEach((t) => {
  ...
  t.context.OP = require('@ellieproject/nellie/mos6502/operations/BVS');
});
...
test('execute() should alter PC when V is set (add)', (t) => {
...
test('execute() should alter PC when V is set (add page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('V', 1);
...
test('execute() should alter PC when V is set (sub)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('V', 1);
...
test('execute() should alter PC when V is set (sub page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('V', 1);
...
test('execute() should not alter PC when V is clear', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
