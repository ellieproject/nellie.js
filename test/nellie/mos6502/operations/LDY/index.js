/* test/nellie/mos6502/operations/LDY/index.js
 *
 */
...
test.beforeEach((t) => {
  ...
  t.context.OP = require('@ellieproject/nellie/mos6502/operations/LDY');
});
...
test('exec() ABSOLUTE_X mode should load Y from PC', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  ...
  t.is(MOS6502.register.y.get(), 0xAA);
});
...
test('exec() ABSOLUTE mode should load Y from PC', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.pc.set(0x0000);
...
test('exec() IMMEDIATE mode should load Y from PC', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.pc.set(0x0000);
  MOS6502.memory.main.data[0x0001] = 0xFF;
...
test('exec() ZERO_PAGE_X mode should load Y from PC', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.pc.set(0x0000);
  MOS6502.register.x.set(0xFF);
...
test('exec() ZERO_PAGE mode should load Y from PC', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.pc.set(0x0000);
  MOS6502.memory.main.data[0x0001] = 0x80;
