/* test/nellie/mos6502/operations/AND/absolute_x.test.js
 *
 * tests for the ABSOLUTE_X mode of the AND operation
...
test.beforeEach((t) => {
  ...
  t.context.MODE    = t.context.OP.mode.ABSOLUTE_X; // this is an opcode
});

// sets accumulator to 0xFF (can AND with anything)
// initializes the PC to 0x0010
// sets up an ABSOLUTE_X AND (memory address: 0x0100 + 0x00FF)
...
// sets accumulator to 0xFF (can AND with anything)
// initializes the PC to 0x0010
// sets up an ABSOLUTE_X AND (memory address: 0x01FF + 0x0001)
...
test('exec(ABSOLUTE_X...) should return this', (t) => {
  const MOS6502 = t.context.MOS6502;
...
test('exec(ABSOLUTE_X...) should alter accumulator', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP      = t.context.OP;
  const MODE    = t.context.MODE;
  setup_0x01FF(t);
...
test('exec(ABSOLUTE_X...) should alter accumulator (page boundary)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP      = t.context.OP;
  const MODE    = t.context.MODE;
  setup_0x0200_page(t);
