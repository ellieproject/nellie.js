/* test/nellie/mos6502/operations/LSR/index.test.js
 *
 */
...
test.beforeEach((t) => {
  ...
  t.context.OP = require('@ellieproject/nellie/mos6502/operations/LSR');
});
...
test('execute() should shift B right (no carry)', (t) => {
...
test('execute() should shift B right (carry)', (t) => {
  const MOS6502 = t.context.MOS6502;
...
test('execute() should set C flag to 0', (t) => {
  const MOS6502 = t.context.MOS6502;
  MOS6502.register.p.bitSet('C', 1); // force to 1
  MOS6502.register.b.set(0x00); // 0th bit 0
  t.context.OP.execute(MOS6502);
...
test('execute() should set C flag to 1', (t) => {
  const MOS6502 = t.context.MOS6502;
  MOS6502.register.p.bitSet('C', 0); // force to 0
  MOS6502.register.b.set(0x01); // 0th bit 1
  t.context.OP.execute(MOS6502);
...
test('execute() should never set N flag to 1', (t) => {
  // since a 0 is always shifted down, 7th bit will never be a 1
  const MOS6502 = t.context.MOS6502;
  MOS6502.register.p.bitSet('C', 1); // force to 1
