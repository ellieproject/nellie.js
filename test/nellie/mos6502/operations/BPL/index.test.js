/* test/nellie/mos6502/operations/BPL/index.test.js
 *
 */
...
test('execute() should alter PC when N is clear (add)', (t) => {
...
test('execute() should alter PC when N is clear (add page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('N', 0);
...
test('execute() should alter PC when N is clear (sub)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('N', 0);
...
test('execute() should alter PC when N is clear (sub page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('N', 0);
...
test('execute() should not alter PC when N is set', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
