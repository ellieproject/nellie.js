/* test/nellie/mos6502/operations/BNE/index.test.js
 *
 */
...
test.beforeEach((t) => {
  ...
  t.context.OP = require('@ellieproject/nellie/mos6502/operations/BNE');
});
...
test('execute() should alter PC when Z is clear (add)', (t) => {
...
test('execute() should alter PC when Z is clear (add page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('Z', 0);
...
test('execute() should alter PC when Z is clear (sub)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('Z', 0);
...
test('execute() should alter PC when Z is clear (sub page)', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
  MOS6502.register.p.bitSet('Z', 0);
...
test('execute() should not alter PC when Z is set', (t) => {
  const MOS6502 = t.context.MOS6502;
  const OP = t.context.OP;
